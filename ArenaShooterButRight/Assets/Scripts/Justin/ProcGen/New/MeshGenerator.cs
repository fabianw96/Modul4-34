        ...
        public static MeshData GenerateTerrainMesh(float[,] heightMap, float meshHeightMultiplier, AnimationCurve meshHeightCurve)
        {
            ...
            for (int y = 0; y < height - 1; y++)
            {
                for (int x = 0; x < width - 1; x++)
                {
                    ...
                    new Vector3(x + 1, meshHeightCurve.Evaluate(heightMap[x + 1, y]) * meshHeightMultiplier, y),
                    new Vector3(x, meshHeightCurve.Evaluate(heightMap[x, y + 1]) * meshHeightMultiplier, y + 1),
                    new Vector3(x + 1, meshHeightCurve.Evaluate(heightMap[x + 1, y + 1]) * meshHeightMultiplier, y + 1)
                    };

                    // Add the two triangles of the Quad
                    meshData.AddTriangle(quadVertices[0], quadVertices[3], quadVertices[2]);
                    meshData.AddTriangle(quadVertices[0], quadVertices[1], quadVertices[3]);
                }
            }
